...
describe('Chatbot server', () => {
  ...
  const installationIncidentCategories = [
    'Accidental',
    'Safer Use',
    'Overdose',
    'Other',
  ...
  describe('POST request: power automate button press', () => {
    ...
    it('should return 200 to a valid request', async () => {
      ...
      let response = await chai
        .request(server)
        .post('/')
        .send(unit1FlicRequest_SingleClick)
      response = await chai
        .request(server)
        .post('/')
        .send(unit1FlicRequest_SingleClick)
    ...
    it('should not confuse button presses from different rooms', async () => {
      ...
      response = await chai
        .request(server)
        .post('/')
        .send(unit2FlicRequest_SingleClick)
      response = await chai
        .request(server)
        .post('/')
        .send(unit2FlicRequest_SingleClick)
    ...
    it('should count button presses accurately during an active session', async () => {
      ...
      response = await chai
        .request(server)
        .post('/')
        .send(unit1FlicRequest_DoubleClick)
      response = await chai
        .request(server)
        .post('/')
        .send(unit1FlicRequest_DoubleClick)
  ...
  describe('POST request: flic button press', () => {
    ...
    it('should log a valid request when given the correct API key', async () => {
      ...
      expect(helpers.log).to.have.been.calledWith(
        `VALID api key from '${buttonName}' (${unit1SerialNumber})`
      )
    })
    ...
    it('should log an invalid request when given an incorrect API key', async () => {
      ...
      await chai
        ...
        .send()
      expect(helpers.log).to.have.been.calledWith(
        `INVALID api key from '${buttonName}' (${unit1SerialNumber})`
      )
    })
  ...
  describe('POST request: twilio message', () => {
    ...
    it('should return ok to a valid request and advance the session appropriately', async () => {
      ...
      expect(
        sessions[0].state,
        'state after initial button press'
      ).to.deep.equal(ALERT_STATE.STARTED)
      ...
      expect(
        sessions[0].state,
        'state after initial staff response'
      ).to.deep.equal(ALERT_STATE.WAITING_FOR_CATEGORY)
      ...
      expect(
        sessions[0].state,
        'state after staff have categorized the incident'
      ).to.deep.equal(ALERT_STATE.WAITING_FOR_DETAILS)
      ...
      expect(
        sessions[0].state,
        'state after staff have provided incident notes'
      ).to.deep.equal(ALERT_STATE.COMPLETED)
      ...
      expect(
        sessions[0].state,
        'state after new button press from a different unit'
      ).to.deep.equal(ALERT_STATE.STARTED)
    ...
    it('should send a message to the fallback phone number if enough time has passed without a response', async () => {
      ...
      expect(
        sessions[0].state,
        'state after reminder timeout has elapsed'
      ).to.deep.equal(ALERT_STATE.WAITING_FOR_REPLY)
      ...
      expect(sessions[0].fallBackAlertTwilioStatus).to.be.oneOf([
        'queued',
        'sent',
        'delivered',
      ])
