...
const { helpers, twilioHelpers, factories } = require('brave-alert-lib')
...
const expectedArguments = [
  ...successfulPhoneNumbers.map(phoneNumber => {
    return { toPhoneNumber: phoneNumber, fromPhoneNumber: testFromPhoneNumber, twilioMessage: testTwilioMessage }
  }),
  ...failPhoneNumbers.map(phoneNumber => {
    return { toPhoneNumber: phoneNumber, fromPhoneNumber: testBadFromPhoneNumber, twilioMessage: testTwilioMessage }
  }),
]
...
describe('pa.js unit tests: handleMessageClients', () => {
  beforeEach(() => {
    sandbox.stub(db, 'getActiveButtonsClients').returns(activeClients)
  })
  ...
  describe('for a request when Twilio is not operating as expected', () => {
    ...
    it('should respond with all phone numbers in failedToMessage', () => {
      ...
      const allPhoneNumbers = [...successfulPhoneNumbers, ...failPhoneNumbers]

      // create array of to phone numbers
      const toPhoneNumbers = this.res.body.failedToMessage.map(twilioTraceObject => twilioTraceObject.to)
