...
function createSessionFromRow(r) {
  return new SessionState(
    ...
    r.incident_type,
    r.notes,
    r.fallback_alert_twilio_status,
    r.button_battery_level
  )
}
...
function createHubFromRow(r) {
  return new Hub(
    r.system_id,
    r.flic_last_seen_time,
    r.flic_last_ping_time,
    r.heartbeat_last_seen_time,
    r.system_name,
    r.hidden,
    r.sent_alerts,
    r.muted,
...
module.exports.getUnrespondedSessionWithButtonId = async function (
  buttonId,
  client
) {
  ...
  const query =
    'SELECT * FROM sessions WHERE button_id = $1 AND state != $2 AND state != $3 AND state != $4'
  const values = [
    buttonId,
...
module.exports.getMostRecentIncompleteSessionWithPhoneNumber = async function (
  phoneNumber,
  client
) {
  const transactionMode = typeof client !== 'undefined'
  ...
  const query =
    'SELECT * FROM sessions WHERE phone_number = $1 AND state != $2 ORDER BY created_at DESC LIMIT 1'
...
module.exports.getAllSessionsWithButtonId = async function (buttonId, client) {
  ...
  const {
    rows,
  } = await client.query('SELECT * FROM sessions WHERE button_id = $1', [
    buttonId,
  ])
...
module.exports.getRecentSessionsWithInstallationId = async function (
  installationId,
  client
) {
  const transactionMode = typeof client !== 'undefined'
  ...
  const {
    ...
    'SELECT * FROM sessions WHERE installation_id = $1 ORDER BY created_at DESC LIMIT 40',
    [installationId]
  )
...
module.exports.getSessionWithSessionId = async function (sessionId, client) {
  ...
  const { rows } = await client.query('SELECT * FROM sessions WHERE id = $1', [
    sessionId,
  ])
...
module.exports.saveSession = async function (session, client) {
  ...
  const {
    rows,
  } = await client.query('SELECT * FROM sessions WHERE id = $1 LIMIT 1', [
    session.id,
  ])
  if (rows.length === 0) {
    if (!transactionMode) {
    ...
    throw new Error(
      "Tried to save a session that doesn't exist yet. Use createSession() instead."
    )
  }
  const query =
...
module.exports.getButtonWithButtonId = async function (buttonId, client) {
  ...
  const {
    rows,
  } = await client.query('SELECT * FROM registry WHERE button_id = $1', [
    buttonId,
  ])
...
module.exports.getButtonWithSerialNumber = async function (
  serialNumber,
  client
) {
  const transactionMode = typeof client !== 'undefined'
  ...
  const {
    ...
    'SELECT * FROM registry WHERE button_serial_number = $1',
    [serialNumber]
  )
...
module.exports.createButton = async function (
  buttonId,
  installationId,
  unit,
  phoneNumber,
  ...
  await client.query(
    'INSERT INTO registry (button_id, installation_id, unit, phone_number, button_serial_number) VALUES ($1, $2, $3, $4, $5)',
...
module.exports.clearButtons = async function (client) {
  if (!helpers.isTestEnvironment()) {
    helpers.log(
      'warning - tried to clear registry database outside of a test environment!'
    )
    return
  }
  const transactionMode = typeof client !== 'undefined'
...
module.exports.createInstallation = async function (
  name,
...
module.exports.clearInstallations = async function (client) {
  if (!helpers.isTestEnvironment()) {
    helpers.log(
      'warning - tried to clear installations table outside of a test environment!'
    )
    return
  }
  const transactionMode = typeof client !== 'undefined'
...
module.exports.getInstallationWithInstallationId = async function (
  installationId,
  client
) {
  const transactionMode = typeof client !== 'undefined'
  ...
  const {
    rows,
  } = await client.query('SELECT * FROM installations WHERE id = $1', [
    installationId,
  ])
...
module.exports.getInstallationWithSessionId = async function (
  sessionId,
  client
) {
  const transactionMode = typeof client !== 'undefined'
  ...
  const {
    ...
    'SELECT i.* FROM sessions s LEFT JOIN installations i ON s.installation_id = i.id WHERE s.id = $1',
    [sessionId]
  )
...
module.exports.getHubWithSystemId = async function (systemId, client) {
  ...
  const { rows } = await client.query(
    'SELECT * FROM hubs WHERE system_id = $1',
    [systemId]
  )
...
module.exports.saveHeartbeat = async function (
  systemId,
  flicLastSeenTime,
  flicLastPingTime,
  ...
  const {
    rows,
  } = await client.query('SELECT * FROM hubs WHERE system_id = $1 LIMIT 1', [
    systemId,
  ])
  if (rows.length === 0) {
    if (!transactionMode) {
    ...
    throw new Error(
      "Tried to save a heartbeat for a hub that doesn't exist yet."
    )
  }
  const query =
...
module.exports.saveHubMuteStatus = async function (systemId, muted, client) {
  ...
  if (rows.length === 0) {
    ...
    throw new Error(
      "Tried to save mute status in a hub that doesn't exist yet."
    )
  }
...
module.exports.saveHubHideStatus = async function (systemId, hidden, client) {
  ...
  if (rows.length === 0) {
    ...
    throw new Error(
      "Tried to save hide status for a hub that doesn't exist yet."
    )
  }
...
module.exports.saveHubAlertStatus = async function (hub, client) {
  ...
  if (rows.length === 0) {
    ...
    throw new Error(
      "Tried to save alert sent status for a hub that doesn't exist yet."
    )
  }
