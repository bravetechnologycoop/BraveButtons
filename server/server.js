...
const unrespondedSessionReminderTimeoutMillis = helpers.getEnvVar(
  'REMINDER_TIMEOUT_MS'
)
const unrespondedSessionAlertTimeoutMillis = helpers.getEnvVar(
  'FALLBACK_TIMEOUT_MS'
)
...
async function sendButtonPressMessageForSession(session, client) {
  const installation = await db.getInstallationWithInstallationId(
  ...
  if (session.numPresses === 1) {
    const alertInfo = {
      ...
      fallbackMessage: `There has been an unresponded request at ${
        installation.name
...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    viewParams.viewMessage =
      allInstallations.length >= 1
        ? 'Please select an installation'
        : 'No installations to display'
...
app.get('/dashboard/:installationId?', async (req, res) => {
  ...
  try {
    const recentSessions = await db.getRecentSessionsWithInstallationId(
      req.params.installationId
    )
    const currentInstallation = await db.getInstallationWithInstallationId(
      req.params.installationId
    )
    const allInstallations = await db.getInstallations()
  ...
  async (req, res) => {
    try {
      ...
      if (validationErrors.isEmpty()) {
        ...
        if (button === null) {
          helpers.log(
            `Bad request: Serial Number is not registered. Serial Number for '${buttonName}' is ${serialNumber}`
          )
          res.status(400).send()
        } else {
...
app.post('/', jsonBodyParser, async (req, res) => {
  try {
    ...
    if (helpers.isValidRequest(req, requiredBodyParams)) {
      ...
      if (button === null) {
        helpers.log(
          `Bad request: UUID is not registered. UUID is ${req.body.UUID}`
        )
        res.status(400).send()
      } else {
...
app.post('/heartbeat', jsonBodyParser, async (req, res) => {
  try {
    ...
    const flicLastSeenTime = moment()
      .subtract(req.body.flic_last_seen_secs, 'seconds')
      .toISOString()
    const flicLastPingTime = moment()
