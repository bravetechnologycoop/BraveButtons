...
class BraveAlerterConfigurator {
  ...
  async getAlertSession(sessionId) {
    ...
    const incidentCategoryKeys = this.createIncidentCategoryKeys(
  ...
  async getAlertSessionByPhoneNumber(toPhoneNumber) {
    const session = await db.getMostRecentIncompleteSessionWithPhoneNumber(
      toPhoneNumber
    )
    if (session === null) {
      return null
  ...
  async alertSessionChangedCallback(alertSession) {
    ...
    const session = await db.getSessionWithSessionId(
    ...
    if (session) {
      ...
      if (alertSession.incidentCategoryKey) {
        const installation = await db.getInstallationWithSessionId(
          alertSession.sessionId,
          client
        )
        session.incidentType =
          installation.incidentCategories[alertSession.incidentCategoryKey]
      }
  ...
  getReturnMessage(fromAlertState, toAlertState, incidentCategories) {
    ...
    switch (fromAlertState) {
      ...
      case ALERT_STATE.WAITING_FOR_REPLY:
        returnMessage = this.createResponseStringFromIncidentCategories(
          incidentCategories
        )
        break
      ...
      case ALERT_STATE.WAITING_FOR_CATEGORY:
        if (toAlertState === ALERT_STATE.WAITING_FOR_CATEGORY) {
          returnMessage =
            "Sorry, the incident type wasn't recognized. Please try again."
        } else if (toAlertState === ALERT_STATE.WAITING_FOR_DETAILS) {
          returnMessage =
            'Thank you. If you like, you can reply with any further details about the incident.'
        }
        break
      ...
      case ALERT_STATE.COMPLETED:
        returnMessage =
          "There is no active session for this button. (You don't need to respond to this message.)"
        break
